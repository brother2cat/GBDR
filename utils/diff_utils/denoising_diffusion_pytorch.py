...
import sys
sys.path.append('../../')
from utils.diff_utils.attend import Attend
from utils.diff_utils.fid_evaluation import FIDEvaluation

from utils.diff_utils.version import __version__
from collections import defaultdict
from utils.diff_utils.time_scheduler import get_schedule_jump
from utils.diff_utils.mask import generate_random_mask
...
class GaussianDiffusion(Module):
    ...
    @torch.inference_mode()
    def p_sample_loop_specific_image(self, noisy_image, steps_reverse, return_all_timesteps=False):
        device = self.device

        img = self.normalize(noisy_image.to(device))
        imgs = [img]

        x_start = None

        for t in tqdm(reversed(range(0, steps_reverse)), desc='sampling loop time step', total=self.num_timesteps):
            self_cond = x_start if self.self_condition else None
            img, x_start = self.p_sample(img, t, self_cond)
            imgs.append(img)
    ...
    @torch.inference_mode()
    def inpainting_loop(self, image_with_mask, cover_rate=0.2, return_all_timesteps=False):
        final = None
        image_with_mask = self.normalize(image_with_mask.to(self.device))
        mask = generate_random_mask(image_with_mask.clone(), cover_rate)
        imgs = [image_with_mask]
        for sample in self.inpainting_loop_progressive(
                image_with_mask=image_with_mask,
                cover_rate=cover_rate,
                mask=mask
        ):
            final = sample
            imgs.append(final['sample'])

        ret = final["sample"] if not return_all_timesteps else torch.stack(imgs, dim=1)

        ret = self.unnormalize(ret)
        return ret

    @torch.inference_mode()
    def inpainting_loop_progressive(self, image_with_mask, cover_rate=0.2, mask=None, return_all_timesteps=False):
        device = self.device
        shape = image_with_mask.shape
        image_after_step = torch.randn_like(image_with_mask, device=device)
    ...
    @torch.inference_mode()
    def p_sample_inpainting(self, x, image_with_mask, t, cover_rate, mask=None,
                            clip_denoised=True, denoised_fn=None, pred_xstart=None):
        """
        xt -> xt-1,
        mask: generate random
        x: xt
        image_with_mask: initial image
        """
        noise = torch.randn_like(x)
        mask = mask if mask is not None else generate_random_mask(image_with_mask.clone(), cover_rate)
        # mask =
        if pred_xstart is not None:
            noisy_image_with_mask = self.q_sample(image_with_mask, t)
            x = mask * noisy_image_with_mask + (1 - mask) * x
        ...
        result = {"sample": sample, "pred_xstart": x_start, 'gt': image_with_mask}
    ...
    def p_losses(self, x_start, t, noise=None, offset_noise_strength=None):
        ...
        # Generate very dark or light images easily
