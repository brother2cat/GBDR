...
class ftrojann_version(object):
    # idea : in this attack, this transform just replace the image by the image_serial_id, the real transform does
    # not happen here

    def __init__(self, YUV, channel_list, window_size, magnitude, pos_list) -> None:
        print(
            "in ftrojan_attack_replace_version, the real transform does not happen here, input img, target must be NONE, "
            "only image_serial_id used"
        )
        self.YUV = YUV
    ...
    def __call__(self, img: np.ndarray) -> np.ndarray:
        original_channel = len(img.shape)
        if original_channel == 3:
            pass
        else:
            img = A.ToRGB(p=1)(image=img)['image']
        ...
        if original_channel == 2:
            img = np.clip(img, 0, 255).astype(np.uint8)
            img = A.ToGray(p=1)(image=img)['image']
