...
class our_defense(defense):
    def __init__(self, attack_yaml_path, defense_yaml_path, save_folder_path):
        ...
        # Image cleansing diffusion model
    ...
    def test_traindata_loss(self):
        ...
        h5_file = h5py.File(f"file_path.hdf5", "w")
    ...
    def pre_train(self):
        self.logger.info("Detection model train")
        ...
        for self.current_epoch in range(self.args.preprocess_epochs):
            ...
            self.scheduler.step()
        self.logger.debug(f"")
        torch.save(self.preprocess_model.cpu().state_dict(), "path")

    def load_pre_train(self):
        self.preprocess_model = self.preprocess_model.cpu()
        self.preprocess_model.load_state_dict(torch.load("path"))
        self.preprocess_model = self.preprocess_model.to(self.device)

    def load_diffusion_model(self):
        self.diffusion_model = self.diffusion_model.cpu()
        self.diffusion_model.load_state_dict(torch.load("path"))
    ...
    def load_relabel_train(self):
        ...
        self.relabel_model.load_state_dict(torch.load("path"))
    ...
    def isolate_dataset(self):
        ...
        self.logger.info(f"Classification of poisoned samples"
    ...
    def diffusion_model_train(self):
        self.logger.info("Image cleansing diffusion model train")
        ...
        for epoch in range(self.args.diff_epochs):
            ...
            self.scheduler.step()
        torch.save(self.diffusion_model.cpu().state_dict(), 'path')

    def diffusion_model_purification(self):
        self.logger.info("Image Purification")
    ...
    def relabel_train(self):
        self.logger.info("Discriminator Model Train")
        ...
        self.train_dataloader = DataLoader(self.Dc,
                                           batch_size=self.args.batch_size,
                                           shuffle=True)
        self.logger.info("Set the train_dataloader with Dc")

        for self.current_epoch in range(self.args.epochs):
            self.train_and_test_one_epoch()
            self.scheduler.step()
        torch.save(self.relabel_model.cpu().state_dict(), "path")

    def relabel_adversarial_finetune(self, num_iter):
        self.logger.info("Discriminator Model Adversarial Finetune")
    ...
    def relabel_clean_purity_finetune(self, num_iter):
        self.logger.info("Discriminator Model clean Finetune")
        self.criterion = nn.CrossEntropyLoss().to(self.device)
    ...
    def relabel_finetune_iteration(self):
        self.logger.info(f"Discriminator Model Iteratively Finetune")
    ...
    def relabelDc_reconDb_target_model_train(self):
        self.logger.info("Train the target model using Classical Supervised Training")
        ...
        Db_recon = h5py_recon_relabel_image_other_info(f"path")
        Relabel_Dc = h5py_recon_relabel_image_other_info(f"path")
        ...
        for self.current_epoch in range(self.args.epochs):
            self.train_and_test_one_epoch()
            self.scheduler.step()
        torch.save(self.target_model.cpu().state_dict(), "path")

    def label_purify(self):
        self.logger.info("Label Purification")
        self.model = self.relabel_model.to(self.device)
        Db_recon = h5py_recon_image_other_info(f"path")
        Db_recon = dataset_wrapper_with_transform(Db_recon, self.clean_train_image_transform)
...
if __name__ == '__main__':
    a = our_defense(attack_yaml_path="path",
                    defense_yaml_path="path",
                    save_folder_path="path")
    a.pre_train()
    a.isolate_dataset()
    a.diffusion_model_train()
    a.diffusion_model_purification()
    a.relabel_train()
